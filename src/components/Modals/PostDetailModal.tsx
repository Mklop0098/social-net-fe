...
export const PostDetail: React.FC<PostDetailProps> = (props) => {
  ...
  const getMessageReceive = (
    messList: MessReceiveType[]
  ): MessReceiveType[] => {
    let tmp: MessReceiveType[] = [];
    messList.map((mess) => {
      const exist1 = tmp.find(
        (item) => item.from === mess.from && item.to === mess.to
      );
      const exist2 = tmp.find(
        (item) => item.from === mess.to && item.to === mess.from
      );
      ...
      if (!exist1 && !exist2) {
        ...
        if (event1 > event) {
          const change = tmp.filter(
            (item) => item.createAt !== exist1.createAt
          );
          change.push(mess);
          tmp = change;
        }
        ...
        if (event1 > event) {
          const change = tmp.filter(
            (item) => item.createAt !== exist2.createAt
          );
          change.push(mess);
          tmp = change;
        }
  ...
  useEffect(() => {
    if (
      newChat.from &&
      !activeChatList.find((user) => user.userId === newChat.from)
    ) {
      ...
      <div className="grid grid-cols-5 w-[100vw] h-[100vh]">
        ...
        <div className="flex flex-col relative">
          <div className="flex flex-row justify-end items-center pl-4 h-[60px] pr-4 shadow-md">
            ...
            >
              <FaBell size={20} />
              {(haveUnreadNotify(notifies) || newNotify) && (
                <div className="w-3 h-3 bg-red-500 absolute top-0 right-0 rounded-full"></div>
              )}
            </div>
            <div
            ...
            >
              <div className="flex flex-col px-2">
                <div className="text-xl px-2 font-bold pt-4 pb-2">
                  Doan Chat
                </div>
                {messageReceive.length > 0 ? (
                  messageReceive.map((mess, key) => (
                    <div
                      key={key}
                      className="hover:bg-gray-100 w-full p-3 flex flex-row items-center cursor-pointer"
                      onClick={() => {
                        addChatList(
                          mess.from !== currentUser._id ? mess.from : mess.to
                        );
                        setShowMess(false);
                      }}
                    >
                      <div
                        className="w-14 h-14 rounded-full bg-gray-300 p-3 mr-3"
                        style={{
                          backgroundImage: `url(${
                            getCurrentFriend(
              ...
              <div className="flex flex-col justify-between items-start p-3">
                <div className="shadow-[rgba(50,_50,_105,_0.15)_0px_2px_5px_0px,_rgba(0,_0,_0,_0.05)_0px_1px_1px_0px] w-full flex flex-col justify-center">
                  <div className="flex flex-row items-center p-4">
                    <div
                      className="bg-gray-500 rounded-full w-12 h-12 mr-4 overflow-hidden"
                      style={{
                        backgroundImage: `url(${currentUser.avatar})`,
                        backgroundPosition: "center",
                        backgroundSize: "cover",
                  ...
                  <a href={`/${currentUser._id}`}>
                    <div className="p-4 text-[--primary-color] cursor-pointer">
                      Xem trang cá nhân
                    </div>
                  </a>
                </div>
            ...
            >
              <div className="flex flex-col justify-between items-start flex flex-col max-h-[80vh]">
                ...
                {notifies.length > 0 ? (
                  notifies.reverse().map((notify, key) => (
                    <a href={"/friends"} key={key}>
                      <div className="hover:bg-gray-100 w-full p-3 flex flex-row items-center cursor-pointer">
                        <div
                          className="w-14 h-14 rounded-full bg-gray-300 overflow-hidden mr-3"
                          style={{
                            backgroundImage: `url(${
                              getCurrentFriend(notify.from).avatar
                            })`,
                            backgroundPosition: "center",
                            backgroundSize: "cover",
                          }}
